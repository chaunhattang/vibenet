...
export const SendMomentModal: React.FC<SendMomentModalProps> = ({
  ...
  const handlePickFromGallery = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        setPickedImage({ uri: asset.uri, type: asset.mimeType || 'image/jpeg', name: fileName });
      }
    } catch (err) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      Alert.alert('Could not send moment', err instanceof Error ? err.message : 'Please try again.');
    } finally {
      setIsSending(false);
    }
  };
        ...
        <KeyboardAvoidingView
          ...
          <View style={styles.header}>
            ...
            <TouchableOpacity
              ...
              style={[styles.sendHeaderBtn, !pickedImage && styles.sendHeaderBtnDisabled]}>
              <Text style={styles.sendHeaderText}>{isSending ? 'Sending...' : 'Blast'}</Text>
            </TouchableOpacity>
          </View>
          ...
          <ScrollView
            ...
            <View style={styles.viewfinderWrap}>
              {pickedImage ? (
                <Image source={{ uri: pickedImage.uri }} style={styles.momentPreview} contentFit="cover" />
              ) : (
                <View style={styles.emptyPreview}>
                  <Feather name="image" size={32} color="rgba(255,255,255,0.4)" />
                  <Text style={styles.emptyPreviewText}>Pick a photo to send</Text>
                </View>
              )}
